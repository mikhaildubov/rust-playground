...
/// Find indices into `text` which point to the start of substrings equal to `subject`
pub fn find_substring_indices(text: &str, subject: &str) -> Vec<usize> {
...
fn simple_test1() {
    let text = "ababacaba";
    let subject = "ab";
    let result = find_substring_indices(text, subject);
...
fn simple_test3() {
    let text = "cbaebabacd";
    let subject = "abc";
    let result = find_anagram_indices(text, subject);
